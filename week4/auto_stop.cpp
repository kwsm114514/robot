...
void nearly(MyRobot, string, int);
            ...
            for (int i = 0; i < detection.size(); i++) {
                ...
                if (bbox.label == "stop sign" && height > 10 && width > 10) {
                    ...
                    if (stops > 7) {
                        ...
                        robot.wait(0.1);
                        // 左すれすれに動く。
                        nearly(robot, camera, "left", 5, true);
                        printf("left end");
                        robot.end();
                    } else if (center_x > (640 / 2) + 50) {
                        // 右すれすれに動く。
                        nearly(robot, camera, "right", 5, true);
                        printf("right end");
                        robot.end();
...
// どちらかの方向の壁にすれすれに動く。
void nearly(MyRobot &robot, DetectionCamera &camera, string direction, int repeat, bool stop) {
    ...
    for (int i = 0; i < repeat; i++) {
        ...
        if (stop) {
            ...
            for (int i = 0; i < detection.size(); i++) {
                ...
                int width = (bbox.br.x - bbox.ul.x);
                int height = (bbox.br.y - bbox.ul.y);
                if (bbox.label == "stop sign" && height > 150 && width > 90) {
