...
// どちらかの方向の壁にすれすれに動く。
void nearly(MyRobot &robot, const char* direction, int repeat) {
...
int main(int argc, char** argv) {
  // ROS node
    ...
    while (ros::ok() && !robot.is_finished() && !camera.is_finished()) {
        ...
        printf("y: %lf, x: %lf\n", robot.get_y(), robot.get_x());
        printf("tie; %lf\n", robot.get_time());

        // 目的地までの時間を短縮できるので制限時間をつけても問題ないはず。
        if (robot.get_time() > 120) {
            printf("time end\n");
            robot.end();
        }
